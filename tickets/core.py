...
class TicketsCore:
    ...
    async def create_ticket(self, context):
        ...
        cur_users = await self.config.guild(guild).users()
        cur_sessions = await self.config.guild(guild).sessions()

        if author.id in cur_sessions.values:
            await context.send("You already have an active ticket!")
            return

        if len(cur_sessions) >= 10:
            await context.send("There are too many tickets present - 10 - for you to open a new one!")
            return

        cur_users.append(author.id)
        await self.config.guild(guild).users.set(cur_users)
