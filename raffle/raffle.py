...
class Raffle(BaseCog):
    ...
    raffle_defaults = {"DOS": None, "Role": None, "Channel": None, "Raffle": {}}
    ...
    async def start(self, ctx, timer, *, title: str):
        ...
        embed.set_footer(
            text=f"Started by {ctx.author.name} | Winners: {winners} | Ends at {fmt_end} UTC"
        )
    ...
    async def reroll(self, ctx, channel: discord.TextChannel, messageid: int):
        ...
        try:
            ...
            return await ctx.send(
                "Nice try slim. You can't add a reaction to a random msg and think that I am "
    ...
    async def start_checks(self, ctx, timer, title):
        ...
        if len(title) > 35:
            ...
            await ctx.send(
                "A raffle is already in progress. End or cancel the "
    ...
    async def start_questions(self, ctx):
        ...
        resp1 = await ctx.bot.wait_for(
            "message", timeout=60, check=lambda m: m.author == ctx.author and len(m.content) < 200
        )
        info = resp1.content
        await resp1.delete()
        await q1.delete()
        q2 = await ctx.send(
            "Please set how many winners are pulled. Must be in the range of 1-6.\nNote: If there are "
            "more winners than entries, I will make everyone a winner."
        )
        resp2 = await ctx.bot.wait_for(
            "message",
            timeout=30,
            check=lambda m: (
                m.author == ctx.author and m.content.isdigit() and 0 < int(m.content) <= 6
            ),
        )
    ...
    async def pick_winner(self, guild, channel, msg):
        ...
        if not winners:
            await channel.send(
                "It appears there were no valid entries, so a winner "
