    ...
    async def on_member_update(self, before, after):
        if before.guild.id == 330432627649544202:
            ...
            if before.roles == after.roles:
                ...
                len(list(set(after.roles).intersection(patroles))) > 0
                and discord.utils.get(server.roles, name="Patreon Supporters") not in after.roles
            ):
                ...
                len(list(set(after.roles).intersection(patroles))) == 0
                and discord.utils.get(server.roles, name="Patreon Supporters") in after.roles
            ):
                await asyncio.sleep(1)
                await after.remove_roles(
                    discord.utils.get(server.roles, name="Patreon Supporters")
                )
                return
        else:
            return
