...
class Shop(BaseCog):
    ...
    member_defaults = {"Inventory": {}, "Trading": True}
    ...
    async def buy(self, ctx, *purchase):
        ...
        if not col:
            return await ctx.send(
                "Either no items have been created, or you need a " "higher role."
            )
        if purchase:
            try:
                shop, item = purchase
    ...
    async def wipe(self, ctx):
        ...
        await ctx.send(
            "You are about to delete all shop and user data from the bot. Are you "
                ...
                for item in shops[shop]["Items"].values():
                    if item["Type"] != "auto":
                        try:
                            ...
                            continue
            await ctx.send(
                "All items in {} have had their quantities increased by {}."
                "".format(shop, amount)
            )
        else:
    ...
    async def mode(self, ctx):
        ...
        await ctx.send(
            "Shop is currently set to {} mode. Would you like to change to {} "
    ...
    async def assign_role(self, ctx, instance, item, role_name):
        ...
        if role is None:
            await ctx.send(
                "Could not assign the role, {}, because it does not exist "
    ...
    async def edit_shop(self, ctx, instance):
        ...
        if choice.content.lower() == "name":
            ...
            await ctx.send(
                "{} is now restricted to only users "
    ...
    async def delete_shop(self, ctx, instance):
        ...
        await ctx.send(
            "Are you sure you wish to delete {} and all of its " "items?".format(name.content)
        )
        choice = await ctx.bot.wait_for("message", timeout=25, check=Checks(ctx).confirm)
    ...
    async def order(self, shop, item):
        ...
        try:
            ...
            return await self.ctx.send(
                "You cannot afford {}x {} for {} {}. Transaction "
...
class ItemManager:
    ...
    async def delete(self):
        ...
        await self.ctx.send(
            "Are you sure you want to delete {} from {}?".format(item.content, shop.content)
        )
    ...
    async def set_quantity(self, _type=None, item=None, shop=None):
        if _type == "auto":
            return await self.ctx.send(
                "You can't change the quantity of an auto item. The "
    ...
    async def add(self, data, shop, item, new_allowed=False):
        async with self.instance.Shops() as shops:
            if shop not in shops:
                ...
                log.error(
                    "{} could not be added to {}, " "because it does not exist.".format(item, shop)
                )
            elif item in shops[shop]["Items"]:
                log.error(
                    "{} was not added because that item already exists in {}."
                    "".format(item, shop)
                )
            else:
...
class Parser:
    ...
    def type_checks(self, idx, row, messages):
        if row["Type"].lower() not in ("basic", "random", "auto", "role"):
            ...
            log.warning(
                "Row {} was not added because the type is a role, but no role was set"
                ".".format(idx)
            )
            return False
        elif (
            row["Type"].lower() == "role"
            and discord.utils.get(self.ctx.message.guild.roles, name=row["Role"]) is None
        ):
            log.warning(
                "Row {} was not added because the {} role does not exist on "
                "the server.".format(idx, row["Role"])
            )
            return False
        elif row["Type"].lower() == "auto" and int(row["Qty"]) == 0:
            log.warning(
                "Row {} was not added because auto items cannot have an infinite quantity."
                "".format(idx)
            )
            return False
            ...
            log.warning(
                "Row {} was not added because one of the messages exceeds 2000 characters."
                "".format(idx)
            )
            return False
