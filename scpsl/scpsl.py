...
class SCPSL(commands.Cog):
    ...
    async def status(self, ctx, server_alias: str = None):
        ...
        if server_alias is None and not local_mode:
            title = (
                "__List of Currently Registered Servers " + self.bot.user.name + " is Tracking.__"
            )
    ...
    async def register(
        self, ctx, ip: str, endport: int, startport: Optional[int] = 7777, *aliases
    ):
        ...
        if startport > endport:
            try:
                await ctx.send(
                    'Invalid port range. Startport "{startport}" cannot be higher than Endport "{endport}"'
                )
            except discord.Forbidden:
                pass
            return
    ...
    async def global_register(
        self, ctx, ip: str, endport=7777, startport: Optional[int] = 7777, *aliases
    ):
