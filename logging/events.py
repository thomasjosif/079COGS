...
class LoggingEvents:
    ...
    async def on_member_ban(self, guild, author):
        if await self.core._validate_event(guild) and author.id != self.bot.user.id:
            ...
            embed.add_field(name=_('**Member**'),
                            value='**{0.name}#{0.discriminator}** ({0.name} {0.id})'.format(author), inline=False)
            embed.set_footer(text='{}'.format(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')))
            ...
            if await self.core._validate_event(guild) and after.id != self.bot.user.id:
                ...
                if before.name != after.name:
                    embed = discord.Embed(color=self.blue,
                                          description=_('From **{0.name}** ({0.id}) to **{1.name}**').format(before,
                                                                                                             after))
                    embed.set_author(name=_('Name changed'))
                    await self.core._send_message_to_channel(guild, embed=embed)
                if before.nick != after.nick:
                    embed = discord.Embed(color=self.blue,
                                          description=_('From **{0.nick}** ({0.id}) to **{1.nick}**').format(before,
                                                                                                             after))
                ...
                if await self.core._validate_event(guild) and author.id != self.bot.user.id:
                    ...
                    if message.attachments:
                        for attachment in message.attachments:
                            filename = await self.core.downloadattachment(attachment.url,
                                                                          attachment.filename,
                                                                          message.id)
                            message = _('Attachment file for message: {}').format(message.id)
                            await self.core._send_message_to_channel(guild,
                                                                     content=message,
    ...
    async def on_guild_role_update(self, before, after):
        ...
        if await self.core._validate_event(guild):
            ...
            if before.color != after.color:
                embed = discord.Embed(color=self.blue)
                embed.set_author(name=_('Role color for {0.name}'
                                        'changed from {0.color} to {1.color}').format(before, after))
                embed.set_footer(text='{}'.format(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')))
                await self.core._send_message_to_channel(guild, embed=embed)
        ...
        if await self.core._ignore(guild, author=author):
            if await self.core._validate_event(guild):
                if not before.channel and after.channel:
                    embed = discord.Embed(color=self.blue)
                    embed.set_author(name=_('{0.name} joined voice channel #{1.channel}').format(author, after),
                                     icon_url=author.avatar_url)
                    embed.set_footer(text='{}'.format(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')))
                    await self.core._send_message_to_channel(guild, embed=embed)
                elif before.channel and not after.channel:
                    embed = discord.Embed(color=self.blue)
                    embed.set_author(name=_('{0.name} left voice channel #{1.channel}').format(author, before),
                                     icon_url=author.avatar_url)
                    embed.set_footer(text='{}'.format(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')))
                    await self.core._send_message_to_channel(guild, embed=embed)
