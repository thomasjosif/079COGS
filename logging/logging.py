...
class Logging(LoggingEvents):
    ...
    async def _logging_set_event_enable(self, context, event_type: str, channel: discord.TextChannel):
        '''
        Enable an event
        '''
        guild = context.guild
    ...
    async def _logging_ignore(self, context):
        ...
        Ignore
        '''
        if context.invoked_subcommand is None or context.invoked_subcommand == self._logging_ignore:
