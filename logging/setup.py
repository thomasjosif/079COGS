...
class LoggingSetup:
    ...
    async def auto_setup(self):
        ...
        big_brother_category = await self.context.guild.create_category(
            "Logs",
            reason=_("Logging needs this to put all event channels in."),
            overwrites=overwrites,
        )

        await message.edit(content=_("Creating event channels..."))

        member_event_channel = await self.context.guild.create_text_channel(
            "member-events",
            category=big_brother_category,
            reason=_("Logging will put all member events in this channel."),
        )
        message_event_channel = await self.context.guild.create_text_channel(
            "message-events",
            category=big_brother_category,
            reason=_("Logging will put all message events in this channel."),
        )
        guild_event_channel = await self.context.guild.create_text_channel(
            "server-events",
            category=big_brother_category,
            reason=_("Logging will put all server events in this channel."),
        )
        mod_event_channel = await self.context.guild.create_text_channel(
            "mod-events",
            category=big_brother_category,
            reason=_("Logging will put all mod events in this channel."),
        )
    ...
    async def setup(self):
        ...
        embed = discord.Embed(
            title=_("**Welcome to the setup for Logging**"),
        ...
        # Member events
        events["on_member_join"]["enabled"] = await self._yes_no(
            _("Do you want to track members joining? [y]es/[n]o"), self.context
        )
        if events["on_member_join"]["enabled"]:
            events["on_member_join"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_member_ban"]["enabled"] = await self._yes_no(
            _("Do you want to track members being banned? [y]es/[n]o"), self.context
        )
        if events["on_member_ban"]["enabled"]:
            events["on_member_ban"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_member_unban"]["enabled"] = await self._yes_no(
            _("Do you want to track members being unbanned? [y]es/[n]o"), self.context
        )
        if events["on_member_unban"]["enabled"]:
            events["on_member_unban"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_member_remove"]["enabled"] = await self._yes_no(
            _("Do you want to track members leaving this server? [y]es/[n]o"), self.context
        )
        if events["on_member_remove"]["enabled"]:
            events["on_member_remove"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_member_update"]["enabled"] = await self._yes_no(
            _("Do you want to track member changes? [y]es/[n]o"), self.context
        )
        if events["on_member_update"]["enabled"]:
            events["on_member_update"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_voice_state_update"]["enabled"] = await self._yes_no(
            _("Do you want to track voice channel changes? [y]es/[n]o"), self.context
        )
        if events["on_voice_state_update"]["enabled"]:
            events["on_voice_state_update"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        # Message events
        events["on_message_delete"]["enabled"] = await self._yes_no(
            _("Do you want to track message deletion? [y]es/[n]o"), self.context
        )
        if events["on_message_delete"]["enabled"]:
            events["on_message_delete"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_raw_bulk_message_delete"]["enabled"] = await self._yes_no(
            _("Do you want to track bulk message deletion? [y]es/[n]o"), self.context
        )
        if events["on_raw_bulk_message_delete"]["enabled"]:
            events["on_raw_bulk_message_delete"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_message_edit"]["enabled"] = await self._yes_no(
            _("Do you want to track message editing? [y]es/[n]o"), self.context
        )
        if events["on_message_edit"]["enabled"]:
            events["on_message_edit"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        # Server events
        events["on_guild_channel_create"]["enabled"] = await self._yes_no(
            _("Do you want to track channel creation? [y]es/[n]o"), self.context
        )
        if events["on_guild_channel_create"]["enabled"]:
            events["on_guild_channel_create"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_guild_channel_delete"]["enabled"] = await self._yes_no(
            _("Do you want to track channel deletion? [y]es/[n]o"), self.context
        )
        if events["on_guild_channel_delete"]["enabled"]:
            events["on_guild_channel_delete"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_guild_channel_update"]["enabled"] = await self._yes_no(
            _("Do you want to track channel updates? [y]es/[n]o"), self.context
        )
        if events["on_guild_channel_update"]["enabled"]:
            events["on_guild_channel_update"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_guild_update"]["enabled"] = await self._yes_no(
            _("Do you want to track server updates? [y]es/[n]o"), self.context
        )
        if events["on_guild_update"]["enabled"]:
            events["on_guild_update"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_guild_role_create"]["enabled"] = await self._yes_no(
            _("Do you want to track role creation? [y]es/[n]o"), self.context
        )
        if events["on_guild_role_create"]["enabled"]:
            events["on_guild_role_create"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_guild_role_delete"]["enabled"] = await self._yes_no(
            _("Do you want to track role deletion? [y]es/[n]o"), self.context
        )
        if events["on_guild_role_delete"]["enabled"]:
            events["on_guild_role_delete"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_guild_role_update"]["enabled"] = await self._yes_no(
            _("Do you want to track role updates? [y]es/[n]o"), self.context
        )
        if events["on_guild_role_update"]["enabled"]:
            events["on_guild_role_update"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

            # Warning events
            events["on_kick"]["enabled"] = await self._yes_no(
                _("Do you want to track member kicks? [y]es/[n]o"), self.context
            )
        if events["on_kick"]["enabled"]:
            events["on_kick"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )

        events["on_ban"]["enabled"] = await self._yes_no(
            _("Do you want to track member bans? [y]es/[n]o"), self.context
        )
        if events["on_ban"]["enabled"]:
            events["on_ban"]["channel"] = await self._what_channel(
                _("Which channel should I use for this event? (please mention the channel)"),
                self.context,
            )
