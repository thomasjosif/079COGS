...
class Welcome(commands.Cog):
    ...
    """
        ...
        guild = ctx.guild
        target_state = (
            on_off if on_off is not None else not (await self.config.guild(guild).enabled())
        )
    ...
    async def __toggle(self, ctx: commands.Context, on_off: bool, event: str):
        ...
        await ctx.send(
            ("{} notices are now {}." "").format(
                event.capitalize(), ENABLED if target_state else DISABLED
            )
        )
    ...
    async def __toggledelete(self, ctx: commands.Context, on_off: bool, event: str):
        ...
        await ctx.send(
            ("Deletion of previous {} notice is now {}." "").format(
                event, ENABLED if target_state else DISABLED
            )
        )
    ...
    async def __msg_del(self, ctx: commands.Context, event: str):
        ...
        async with self.config.guild(guild).get_attr(event).messages() as messages:
            if len(messages) == 1:
                await ctx.send(
                    ("I only have one {} message format, so I can't let you delete it." "").format(
                        event
                    )
                )
                return
            ...
            await ctx.send(
                ("Please enter the number of the {} message format you wish to delete." "").format(
                    event
                )
            )
            ...
            try:
                ...
                await ctx.send(
                    ("Okay, I won't remove any of the {} message formats." "").format(event)
                )
                return
            else:
    ...
    async def __get_number_input(
        self, ctx: commands.Context, maximum: int, minimum: int = 0
    ) -> int:
    ...
    async def __delete_message(self, guild: discord.Guild, message_id: int):
        ...
        try:
            ...
            log.warning(
                ("Failed to delete message (ID {}): insufficient permissions" "").format(
                    message_id
                )
            )
        except:
        ...
        *,
        ...
        try:
            return await channel.send(
                format_str.format(
                    member=user, server=guild, bot=user, count=count or "", plural=plural
                )
            )
        except discord.Forbidden:
            log.error(
            ...
            log.error(
                ("Failed to send {} message to channel ID {1.id} (server ID {2.id})" "").format(
                    event, channel, guild
                )
            )
            return None
    ...
    async def __dm_user(self, member: discord.Member):
        ...
        try:
            ...
            log.error(
                ("Failed to send DM to member ID {0.id} (server ID {1.id})" "").format(
                    member, member.guild
                )
            )
